...
local isDraggingScript = false
local pendingDragInput = nil
local pendingDragItemId = nil
local pendingDragTick = 0
...
local dragOffsetX = 0
local dragOffsetY = 0
local dragCurrentList = {}
local dragCurrentIndex = 1
...
ToggleButton.Position = UDim2.new(1, -120, 0.5, -15)
...
RemoveUICorner.CornerRadius = UDim.new(1, 0)
...
ConfirmRemoveCorner.CornerRadius = UDim.new(0, 14)
...
ConfirmRemoveYesCorner.CornerRadius = UDim.new(1, 0)
...
ConfirmRemoveNoCorner.CornerRadius = UDim.new(1, 0)
...
NameCorner.CornerRadius = UDim.new(1, 0)
...
-- ============================================================
-- LIST CONTAINER & ADVANCED DRAG SYSTEM
-- ============================================================
...
ListTitle.Text = "📂 My Scripts (Hold to Drag)"
...
local function processDragReorder()
    if not dragClone then return end
    
    local canvasY = dragClone.AbsolutePosition.Y - ScrollFrame.AbsolutePosition.Y + ScrollFrame.CanvasPosition.Y
    local targetIndex = math.floor((canvasY + 21) / 42) + 1 
    targetIndex = math.clamp(targetIndex, 1, #dragCurrentList)
    
    if targetIndex ~= dragCurrentIndex then
        local frame = table.remove(dragCurrentList, dragCurrentIndex)
        table.insert(dragCurrentList, targetIndex, frame)
        for i, f in ipairs(dragCurrentList) do
            f.LayoutOrder = i
        end
...
local function startDragging(input, id)
    isDraggingScript = true
    activeDragInput = input
    pendingDragInput = nil
    
    -- Disables scrolling frame so it doesn't fight your finger
    ScrollFrame.ScrollingEnabled = false
    
    local currentItem = ScrollFrame:FindFirstChild("ScriptItem_" .. id)
    if not currentItem then return end
    
    dragCurrentList = {}
    for _, child in ipairs(ScrollFrame:GetChildren()) do
        if child:IsA("Frame") and child.Name:match("^ScriptItem_") then
            table.insert(dragCurrentList, child)
        end
    end
    table.sort(dragCurrentList, function(a, b) return a.LayoutOrder < b.LayoutOrder end)
    
    for i, f in ipairs(dragCurrentList) do
        if f.Name == "ScriptItem_" .. id then
            dragCurrentIndex = i
            break
        end
    end
    
    dragOffsetX = input.Position.X - currentItem.AbsolutePosition.X
    dragOffsetY = input.Position.Y - currentItem.AbsolutePosition.Y
    
    dragClone = currentItem:Clone()
    dragClone.Parent = ListContainer
    dragClone.Size = UDim2.new(0, currentItem.AbsoluteSize.X, 0, currentItem.AbsoluteSize.Y)
    
    local startX = currentItem.AbsolutePosition.X - ListContainer.AbsolutePosition.X
    local startY = currentItem.AbsolutePosition.Y - ListContainer.AbsolutePosition.Y
    dragClone.Position = UDim2.new(0, startX, 0, startY)
    dragClone.ZIndex = 100
    dragClone.BackgroundTransparency = 0.1
    
    local stroke = Instance.new("UIStroke")
    stroke.Color = themeColor
    stroke.Thickness = 2
    stroke.Parent = dragClone
    
    currentItem.BackgroundTransparency = 0.8
    for _, child in ipairs(currentItem:GetChildren()) do
...
RunService.Heartbeat:Connect(function()
    if pendingDragInput and not isDraggingScript then
        if tick() - pendingDragTick >= 0.2 then
            startDragging(pendingDragInput, pendingDragItemId)
        end
    end
    
    if isDraggingScript and activeDragInput and dragClone then
        local pointerY = activeDragInput.Position.Y
        local topY = ListContainer.AbsolutePosition.Y
        local botY = topY + ListContainer.AbsoluteSize.Y
        
        local scrollSpeed = 0
        if pointerY < topY + 40 then
            scrollSpeed = -3 -- Slowed down significantly for better control
        elseif pointerY > botY - 40 then
            scrollSpeed = 3 -- Slowed down significantly for better control
        end
        
        if scrollSpeed ~= 0 then
            ScrollFrame.CanvasPosition = Vector2.new(0, math.clamp(ScrollFrame.CanvasPosition.Y + scrollSpeed, 0, ScrollFrame.AbsoluteCanvasSize.Y - ScrollFrame.AbsoluteSize.Y))
            processDragReorder()
        end
    end
end)
...
UserInputService.InputChanged:Connect(function(input)
    if pendingDragInput == input and not isDraggingScript then
        if dragStartPos and (input.Position - dragStartPos).Magnitude > 10 then
            pendingDragInput = nil 
        end
    end
    
    if isDraggingScript and activeDragInput == input and dragClone then
        local localX = input.Position.X - ListContainer.AbsolutePosition.X - dragOffsetX
        local localY = input.Position.Y - ListContainer.AbsolutePosition.Y - dragOffsetY
        
        localX = math.clamp(localX, 4, ListContainer.AbsoluteSize.X - dragClone.AbsoluteSize.X - 4)
        localY = math.clamp(localY, 0, ListContainer.AbsoluteSize.Y - dragClone.AbsoluteSize.Y)
        
        dragClone.Position = UDim2.new(0, localX, 0, localY)
        processDragReorder()
    end
end)

UserInputService.InputEnded:Connect(function(input)
    if pendingDragInput == input then
        pendingDragInput = nil
    end
    
    if isDraggingScript and activeDragInput == input then
        ...
        local newSavedScripts = {}
        for _, f in ipairs(dragCurrentList) do
            local id = f.Name:sub(12) 
            for _, data in ipairs(savedScripts) do
                if data.id == id then
                    table.insert(newSavedScripts, data)
                    break
                end
            end
        end
        
        savedScripts = newSavedScripts
        saveScriptsToFile()
        refreshScriptList()
    end
end)
...
ColorPickerCorner.CornerRadius = UDim.new(0, 16)
...
CloseColorCorner.CornerRadius = UDim.new(1, 0)
...
RenameFrameCorner.CornerRadius = UDim.new(0, 14)
...
RenameInputCorner.CornerRadius = UDim.new(1, 0)
RenameInputCorner.Parent = RenameInput

local RenameInputPadding = Instance.new("UIPadding")
RenameInputPadding.PaddingLeft = UDim.new(0, 14)
...
RenameConfirmCorner.CornerRadius = UDim.new(1, 0)
...
RenameCancelCorner.CornerRadius = UDim.new(1, 0)
...
EditFrameCorner.CornerRadius = UDim.new(0, 14)
...
SaveEditCorner.CornerRadius = UDim.new(1, 0)
...
CancelEditCorner.CornerRadius = UDim.new(1, 0)
...
ConfirmYesCorner.CornerRadius = UDim.new(1, 0)
...
ConfirmNoCorner.CornerRadius = UDim.new(1, 0)
...
ResizeHandle.InputBegan:Connect(function(input)
    -- Disabled completely if currently dragging a script!
    if isDraggingScript or pendingDragInput then return end 
...
refreshScriptList = function()
    ...
    for i, scriptData in ipairs(savedScripts) do
        ...
        ScriptItem.Name = "ScriptItem_" .. scriptData.id
        ...
        ScriptName.Size = UDim2.new(1, -110, 1, 0)
        ...
        -- Hold to drag listener
        ScriptItem.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                if input.Position.X < ScriptItem.AbsolutePosition.X + ScriptItem.AbsoluteSize.X - 98 then
                    pendingDragInput = input
                    pendingDragItemId = scriptData.id
                    pendingDragTick = tick()
        ...
        BtnContainer.Size = UDim2.new(0, 98, 1, 0)
        ...
        local ExecBtn = createSmallBtn("▶", Color3.fromRGB(39, 174, 96), 1)
        ExecBtn.MouseButton1Click:Connect(function()
            pcall(function() loadstring(scriptData.code)() end)
        end)

        local RenameBtn = createSmallBtn("🏷", Color3.fromRGB(59, 130, 246), 2)
        RenameBtn.MouseButton1Click:Connect(function()
            for idx, sData in ipairs(savedScripts) do
                if sData.id == scriptData.id then currentRenamingIndex = idx break end
            end
            RenameInput.Text = scriptData.name
            RenameFrame.Visible = true
        end)

        local EditBtn = createSmallBtn("✏", Color3.fromRGB(249, 115, 22), 3)
        EditBtn.MouseButton1Click:Connect(function()
            for idx, sData in ipairs(savedScripts) do
                if sData.id == scriptData.id then currentEditingIndex = idx break end
            end
        ...
        DelBtn.MouseButton1Click:Connect(function()
            for idx, sData in ipairs(savedScripts) do
                if sData.id == scriptData.id then scriptToDeleteIndex = idx break end
            end
